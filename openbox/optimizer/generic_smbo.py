...
from BO.whitebox import WhiteBoxModel
import pickle
...
class SMBO(BOBase):
    ...
    def iterate(self, time_left=None) -> Observation:
        ...
        if self.train:
            performance_scores = self.white_box_model.save_features([config])
        ...
        if self.train:
            with open(f'{self.white_box_model.get_pkl_files()}_label.pkl','ab') as f:
                pickle.dump((objectives, constraints), f)
        self.iteration_id += 1
        # Logging
