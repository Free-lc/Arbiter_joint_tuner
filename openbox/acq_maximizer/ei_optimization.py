...
class RandomScipyOptimizer(AcquisitionFunctionMaximizer):
    ...
    def maximize(
        ...
        # #---------- only for test
        # try:
        #     with open('BO/bar_configs.pkl', 'rb') as f:
        #         bar_configs =  pickle.load(f)
        # except (FileNotFoundError, EOFError):
        #     bar_configs = []
        # whitebox_scores = self.white_box_model.get_scores(bar_configs)
        # whitebox_scores = whitebox_scores.flatten()
        # prf_scores = self.acquisition_function(bar_configs).flatten()
        # try:
        #     with open('visualization/bar/whitebox_scores_2.pkl', 'wb') as f:
        #         pickle.dump(whitebox_scores,f)
        #         print(whitebox_scores)
        # except (FileNotFoundError, EOFError):
        #     print("QAQ")

        # try:
        #     with open('visualization/bar/prf_scores_2.pkl', 'wb') as f:
        #         pickle.dump(prf_scores,f )
        #         print(prf_scores)
        # except (FileNotFoundError, EOFError):
        #     print("QAQ")
        # #----------
        if self.white_box_model is not None: 
            performance_scores = self.white_box_model.get_scores(initial_configs)# 0 or 1
            performance_scores = performance_scores.flatten()
            random_probs = np.random.rand(performance_scores.shape[0])
            performance_scores[random_probs >= trust_probability] = 1
            # use whitebox to guide EIccition_function(initial_configs)
            initial_acqs = self.acquisition_function(initial_configs).flatten() * performance_scores
            initial_acqs = initial_acqs.reshape(-1, 1)
        else:
        ...
        for config in scipy_initial_configs:
            ...
            if len(scipy_configs) == 1:
                scipy_configs = scipy_configs + scipy_configs
            if not scipy_configs:   # empty
                continue
            if self.white_box_model is not None:
                performance_scores = self.white_box_model.get_scores(scipy_configs)# 0 or 1
                performance_scores = performance_scores.flatten()
                random_probs = np.random.rand(performance_scores.shape[0])
                performance_scores[random_probs >= trust_probability] = 1
                # use whitebox to guide EI
                scipy_acqs = self.acquisition_function(scipy_configs).flatten() * performance_scores
                scipy_acqs = scipy_acqs.reshape(-1, 1)
            else:
