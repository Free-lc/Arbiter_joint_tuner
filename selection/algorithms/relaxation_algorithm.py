...
class RelaxationAlgorithm(SelectionAlgorithm):
    def __init__(self, database_connector, parameters=None):
        ...
        self.disk_constraint = self.parameters["budget_MB"]
    ...
    def _calculate_best_indexes(self, workload, valid_workloads):
        ...
        cp_cost = sum([self.cost_evaluation.calculate_cost_multithread(valid_workload, cp, store_size=False) for valid_workload in self.valid_workloads])
        while cp_size > self.disk_constraint:
            ...
            for transformation in self.transformations:
                for (
                    ...
                    relaxed_cost = sum([self.cost_evaluation.calculate_cost_multithread(valid_workload, relaxed, store_size=False) for valid_workload in self.valid_workloads])
                    ...
                    for partition_id in range(self.partition_num):
                        cost += self.cost_evaluation.calculate_cost_multithread(
                            self.valid_workloads[partition_id], best_relaxed, store_size = False
                        )
                    index_size = sum([index.estimated_size for index in best_relaxed])
                    logging.info(f"{time_begin_cal - self.time_begin} s, cost :{cost}, index_size: {index_size}")
                    self.time_begin = time.time()

            cp = best_relaxed
            cp_size = best_relaxed_size
            # if cp_size is None or self.disk_constraint is None:
            #     break
