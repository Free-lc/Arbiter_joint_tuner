...
class AutoAdminAlgorithm(SelectionAlgorithm):
    ...
    def enumerate_combinations(self, workload, candidate_indexes):
        ...
        for partition_id in range(self.partition_num):
            cost += self.cost_evaluation.calculate_cost_multithread(
                self.valid_workloads[partition_id], list(set(indexes)), store_size = False
            )
        index_size = sum([index.estimated_size for index in list(set(indexes))])
    ...
    def _simulate_and_evaluate_cost(self, workload, indexes):
        # cost = self.cost_evaluation.calculate_cost_multithread(workload, indexes, store_size=False)
        cost = sum([self.cost_evaluation.calculate_cost_multithread(valid_workload, indexes, store_size=False) for valid_workload in self.valid_workloads])
