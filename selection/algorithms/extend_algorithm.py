...
class ExtendAlgorithm(SelectionAlgorithm):
    ...
    def _calculate_best_indexes(self, workload, valid_workloads):
        ...
        current_cost = sum([self.cost_evaluation.calculate_cost_multithread(valid_workload, index_combination, store_size=False) for valid_workload in self.valid_workloads])
        # current_cost = self.cost_evaluation.calculate_cost_multithread(
    ...
    def _evaluate_combination(
        ...
        cost = sum([self.cost_evaluation.calculate_cost_multithread(valid_workload, index_combination, store_size=False) for valid_workload in self.valid_workloads])
        ...
        # the size of some multiple index in greenplum is equal to single column index, so the code below will cause bug
        # assert new_index_size_difference != 0, "Index size difference should not be 0!"

        if new_index_size_difference == 0 and new_index.estimated_size != 0:
